...
export default class CKAFKADatasource implements DatasourceInterface {
          ...
          return _.flatMap(extraDimList, (extraDim) => {
            ...
            if (Array.isArray(targetDims)) {
              return targetDims.map(dim => {
                ...
                return { Dimensions: GetDimensions({ ...dimensionObject, [extraDim]: { Name: extraDim, Value: dim } }) }
              });
            }
  ...
  async getConsumerGroups(region, params) {
    ...
    // 从分页数据，获取全量数据
    const rs = await fetchAllFactory(data => {
      return this.doRequest({
        url: this.url + serviceInfo.path,
        data 
      }, serviceInfo.service, { region, action: 'DescribeConsumerGroup' })
    }, params, ['GroupListForMonitor', 'TopicListForMonitor', 'PartitionListForMonitor'])
