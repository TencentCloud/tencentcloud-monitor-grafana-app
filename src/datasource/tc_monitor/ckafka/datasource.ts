...
export default class CKAFKADatasource implements DatasourceInterface {
  ...
  metricFindQuery(query: { action: string, namespace: string, region?: string }, regex?: string) {
    ...
    if (instancesQuery && region) {
      ...
      return this.getVariableInstances(region).then(result => {
        const instanceAlias = CKAFKAInstanceAliasList.indexOf(query[VARIABLE_ALIAS]) !== -1 ? query[VARIABLE_ALIAS] : 'InstanceId';
        const instances: any[] = [];
      ...
      const data = {
        ...
        Instances: _.map(instances, instance => {
          ...
          _.forEach(dimensionObject, (__, key) => {
            ...
            dimensionObject[key] = { Name: key, Value: instance[keyTmp] ??  target.ckafka[keyTmp]}; // 从实例中取，如果取不到，则从界面模型上取

            // 设置instance
            instance[key] = instance[keyTmp] ??  target.ckafka[keyTmp];
          });
          console.log('dimension', dimensionObject);
  ...
  getConsumerGroups(region, params) {
    const serviceInfo = GetServiceAPIInfo(region, 'ckafka');
    return this.doRequest({
      url: this.url + serviceInfo.path,
      data: params
    }, serviceInfo.service, { region, action: 'DescribeConsumerGroup' })
      .then(response => {
        let { Result: { TopicList, GroupList, PartitionListForMonitor } } = response;
        TopicList = TopicList || [];
        GroupList = GroupList || [];
        ...
        return {
          ...
          GroupList: GroupList.map(group => ({ text: group.ConsumerGroupName,value: group.ConsumerGroupName,})),
          PartitionList: PartitionListForMonitor.map(par => ({ text: par.Partition, value: par.Partition }))
        }
      });
  }
