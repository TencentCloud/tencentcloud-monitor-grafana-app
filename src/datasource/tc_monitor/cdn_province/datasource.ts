...
export default class CDNPROVDatasource implements DatasourceInterface {
  ...
  metricFindQuery(query: object, regex?: string | undefined) {
    ...
    if (instancesQuery && region) {
      ...
      return this.getVariableInstances(region, Filters.length > 0 ? { Filters } : undefined).then(result => {
        const instanceAlias = CDNInstanceAliasList.indexOf(query[VARIABLE_ALIAS]) !== -1 ? query[VARIABLE_ALIAS] : 'Domain';
        const instances: any[] = [];
  ...
  query(options: any) {
    const queries = _.filter(options.targets, item => {
      ...
      return (
        ...
        !!item.namespace &&
        !!item.cdn.metricName &&
        !_.isEmpty(ReplaceVariable(this.templateSrv, options.scopedVars, item.cdn.region, false)) &&
        !_.isEmpty(ReplaceVariable(this.templateSrv, options.scopedVars, item.cdn.instance, true))
      );
    }).map(target => {
      // 实例 instances 可能为模板变量，需先获取实际值
      let instances = ReplaceVariable(this.templateSrv, options.scopedVars, target.cdn.instance, true);
      ...
      const region = ReplaceVariable(this.templateSrv, options.scopedVars, target.cdn.region, false);
      const data = {
        StartTime: moment(options.range.from).format(),
        EndTime: moment(options.range.to).format(),
        Period: target.cdn.period || 300,
        Instances: _.map(instances, instance => {
          const dimensionObject = target.cdn.dimensionObject;
          _.forEach(dimensionObject, (__, key) => {
            if (_.has(CDNInvalidDemensions,key)) {
              const keyTmp = CDNInvalidDemensions[key];
        ...
        MetricName: target.cdn.metricName,
      };
