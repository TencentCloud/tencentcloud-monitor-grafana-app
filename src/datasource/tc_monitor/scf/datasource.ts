...
export default class SCFDatasource implements DatasourceInterface {
  ...
  metricFindQuery(query: { action: string, namespace: string, region?: string }, regex?: string) {
    ...
    if (instancesQuery && region) {
      ...
      return this.getVariableInstances(region).then(result => {
        const instanceAlias = SCFInstanceAliasList.indexOf(query[VARIABLE_ALIAS]) !== -1 ? query[VARIABLE_ALIAS] : 'FunctionName';
        const instances: any[] = [];
  ...
  query(options: any) {
    const queries = _.filter(options.targets, item => {
      ...
      let instances = ReplaceVariable(this.templateSrv, options.scopedVars, target.scf.instance, true);
  ...
  getVersions(region, params) {
    ...
    return this.doRequest({
      url: this.url + serviceInfo.path,
      data: params
    }, serviceInfo.service, { region, action: 'ListVersionByFunction' })
      .then(response => {
  ...
  getInstances(region, params = {}) {
    ...
    return this.doRequest({
      url: this.url + serviceInfo.path,
      data: params,
    }, serviceInfo.service, { region, action: 'ListFunctions' })
      .then(response => {
