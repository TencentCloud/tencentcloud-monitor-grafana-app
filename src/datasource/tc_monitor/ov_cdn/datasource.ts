...
export default class OVCDNDatasource implements DatasourceInterface {
  ...
  metricFindQuery(query: object, regex?: string | undefined) {
    ...
    if (instancesQuery && region) {
      ...
      return this.getVariableInstances(region, Filters.length > 0 ? { Filters } : undefined).then(result => {
        const instanceAlias = OVCDNInstanceAliasList.indexOf(query[VARIABLE_ALIAS]) !== -1 ? query[VARIABLE_ALIAS] : 'Domain';
        const instances: any[] = [];
  ...
  query(options: any) {
    const queries = _.filter(options.targets, item => {
      // 过滤无效的查询 target
      return (
        item.ovCdn.hide !== true &&
        !!item.namespace &&
        !!item.ovCdn.metricName &&
        !_.isEmpty(ReplaceVariable(this.templateSrv, options.scopedVars, item.ovCdn.region, false)) &&
        !_.isEmpty(ReplaceVariable(this.templateSrv, options.scopedVars, item.ovCdn.instance, true))
      );
    }).map(target => {
      // 实例 instances 可能为模板变量，需先获取实际值
      let instances = ReplaceVariable(this.templateSrv, options.scopedVars, target.ovCdn.instance, true);
      ...
      const region = ReplaceVariable(this.templateSrv, options.scopedVars, target.ovCdn.region, false);
      const data = {
        StartTime: moment(options.range.from).format(),
        EndTime: moment(options.range.to).format(),
        Period: target.ovCdn.period || 300,
        Instances: _.map(instances, instance => {
          const dimensionObject = target.ovCdn.dimensionObject;
          _.forEach(dimensionObject, (__, key) => {
            if (_.has(OVCDNInvalidDemensions,key)) {
              const keyTmp = OVCDNInvalidDemensions[key];
        ...
        MetricName: target.ovCdn.metricName,
      };
  ...
  getMonitorData(params, region, instances) {
    ...
    .then(response => {
      ...
      //     {
      //       Dimensions: [
      //         {Name: "Domain", Value: "www.test.com"},
      //         {Name: "ProjectId", Value: "1"},
      //       ],
      //       Timestamps: [
      //         1614126000,1614126300,1614126600,1614126900,1614127200
      //       ],
      //       Values: [
      //         0.12,0.665,0.761,0.16,0.27
      //       ]
      //     }
      //   ]
      // }, instances);
  ...
  getInstances(region, params = {}) {
    ...
    return this.doRequest({
      url: this.url + serviceInfo.path,
      data: params,
    }, serviceInfo.service, { region, action: 'DescribeDomains' })
      .then(response => {
  ...
  testDatasource() {
    ...
    return Promise.all([
      ...
      this.doRequest(
        ...
        'cdn',
        { region: 'ap-guangzhou', action: 'DescribeDomains' }
      ),
    ])
      .then(responses => {
        const cvmErr = _.get(responses, '[0].Error', {});
        const monitorErr = _.get(responses, '[1].Error', {});
        const CDNErr = _.get(responses, '[2].Error', {});
        ...
        const CDNAuthFail = _.get(CDNErr, 'Code', '').indexOf('AuthFailure') !== -1;
        if (cvmAuthFail || monitorAuthFail || CDNAuthFail) {
          const messages: any[] = [];
          if (cvmAuthFail) {
          ...
          if (CDNAuthFail) {
            messages.push(`${_.get(CDNAuthFail, 'Code')}: ${_.get(CDNAuthFail, 'Message')}`);
          }
          ...
          return {
            ...
            service: 'cdn',
            status: 'success',
            message: 'Successfully queried the CDN service.',
            title: 'Success',
          };
        }
      })
      .catch(error => {
        let message = 'CDN service:';
        ...
        if (!!_.get(error, 'data.error.code', '')) {
          ...
          message += 'Cannot connect to CDN service.';
        }
        return {
          service: 'cdn',
          status: 'error',
