...
export default class REDISDatasource implements DatasourceInterface {
  Namespace = 'QCE/REDIS';
  url: string;
  ...
  metricFindQuery(query: object) {
    ...
    if (instancesQuery && region) {
      return this.getVariableInstances(region).then(result => {
        const instanceAlias = REDISInstanceAliasList.indexOf(query[VARIABLE_ALIAS]) !== -1 ? query[VARIABLE_ALIAS] : 'InstanceId';
        const instances: any[] = [];
        _.forEach(result, (item) => {
          ...
          if (instanceAliasValue) {
            if (typeof instanceAliasValue === 'string') {
              ...
              instances.push({ text: instanceAliasValue, value: JSON.stringify(_.pick(item, _.concat(REDISInstanceAliasList, ['_InstanceAliasValue']))) });
            } else if (_.isArray(instanceAliasValue)) {
              _.forEach(instanceAliasValue, (subItem) => {
                item._InstanceAliasValue = subItem;
                instances.push({ text: subItem, value: JSON.stringify(_.pick(item, _.concat(REDISInstanceAliasList, ['_InstanceAliasValue']))) });
              });
            }
          }
        });
  ...
  query(options: any) {
    const queries = _.filter(options.targets, item => {
      // 过滤无效的查询 target
      return (
        item.redis.hide !== true &&
        !!item.namespace &&
        !!item.redis.metricName &&
        !_.isEmpty(ReplaceVariable(this.templateSrv, options.scopedVars, item.redis.region, false)) &&
        !_.isEmpty(ReplaceVariable(this.templateSrv, options.scopedVars, item.redis.instance, true))
      );
    }).map(target => {
      // 实例 instances 可能为模板变量，需先获取实际值
      let instances = ReplaceVariable(this.templateSrv, options.scopedVars, target.redis.instance, true);
      ...
      const region = ReplaceVariable(this.templateSrv, options.scopedVars, target.redis.region, false);
      const data = {
        StartTime: moment(options.range.from).format(),
        EndTime: moment(options.range.to).format(),
        Period: target.redis.period || 300,
        Instances: _.map(instances, instance => {
          const dimensionObject = target.redis.dimensionObject;
          _.forEach(dimensionObject, (__, key) => {
            if (_.has(RedisInvalidDemensions,key)) {
              const keyTmp = RedisInvalidDemensions[key];
        ...
        MetricName: target.redis.metricName,
      };
