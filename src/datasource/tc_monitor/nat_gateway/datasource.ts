        ...
        _.forEach(result, (item) => {
          ...
          if (instanceAliasValue) {
            if (typeof instanceAliasValue === 'string') {
              ...
              instances.push({ text: instanceAliasValue, value: JSON.stringify(_.pick(item, _.concat(NATGATEWAYInstanceAliasList, ['_InstanceAliasValue']))) });
            } else if (_.isArray(instanceAliasValue)) {
              _.forEach(instanceAliasValue, (subItem) => {
                item._InstanceAliasValue = subItem;
                instances.push({ text: subItem, value: JSON.stringify(_.pick(item, _.concat(NATGATEWAYInstanceAliasList, ['_InstanceAliasValue']))) });
              });
            }
          }
        });
