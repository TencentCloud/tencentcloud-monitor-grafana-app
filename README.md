...
Web 应用防火墙(WAF) | 参考 [Web 应用防火墙(WAF)实例列表接口文档](https://cloud.tencent.com/document/api/228/41118)。`Namespace` 固定为QCE/WAF，`Action` 固定为`DescribeInstances`。`Region` 为地域参数，可以为特定的地域值，如 `ap-guangzhou`；也可以为变量值，如 `$region`。`InstanceAlias` 为实例的展示字段，默认为 `Domain`，可选值为 `Domain`，`ProjectId`。同时支持单选和多选。 |  Namespace=QCE/WAF&Action=DescribeInstances&Region=$region&InstanceAlias=Domain
